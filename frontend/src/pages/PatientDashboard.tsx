    ...
    return (
      <DashboardLayout>
        <div className="flex flex-col items-center justify-center h-[80vh] gap-4">
          <p className="text-muted-foreground">{t('common.loadError') || 'Could not load dashboard data.'}</p>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6 pb-8">
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-muted-foreground mt-1 flex items-center gap-2">
                ...
                {t('patient.healthStatus')}: <span className="font-medium text-foreground">{data.health_status}</span>
              </p>
              {data.last_check_in && (
                <p className="text-xs text-muted-foreground mt-1 flex items-center gap-1">
                  <Clock size={12} /> {t('patient.lastCheckin')}: {new Date(data.last_check_in).toLocaleDateString()}
                </p>
              )}
            </div>
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <button onClick={openAgentChat} className="w-full glass-card p-5 flex items-center justify-between hover:border-primary/40 transition-all group rounded-2xl">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-gradient-to-r from-primary to-secondary flex items-center justify-center shrink-0">
                <MessageSquare size={16} className="text-white" />
              </div>
              <div className="text-left">
                <p className="text-sm font-semibold text-foreground">{data.pending_question ? t('chat.openCheckin') || 'Complete Check-in' : t('chat.startCheckin') || 'Start Check-in'}</p>
                <p className="text-xs text-muted-foreground">{t('chat.symptomsDesc') || 'Talk to CARA – voice & text'}</p>
              </div>
            </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
            ...
            {data.active_course ? (
              <div className="space-y-4">
                <div className="flex justify-between items-start flex-wrap gap-2">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground flex items-center gap-1">{t('patient.by')} Dr. {data.active_course.doctor_name}</p>
                  </div>
              ...
              <div className="flex flex-col items-center justify-center py-6 gap-2">
                <p className="text-sm text-muted-foreground text-center">
                  {t('patient.noCourse') || 'No active course.'}
                </p>
                <p className="text-xs text-muted-foreground text-center">
                  {t('patient.shareId')} <span className="font-mono bg-muted px-1 rounded">{data.unique_uid}</span> with your doctor.
                </p>
              </div>
            )}
          </div>
          ...
          <div className="glass-card rounded-3xl p-6">
            <h2 className="font-semibold text-foreground mb-4 flex items-center gap-2">
              <Pill size={18} className="text-secondary" /> {t('patient.medications') || "Today's Medications"}
            </h2>
          ...
          <div className="glass-card rounded-3xl p-6">
            ...
            <div className="space-y-4">
              {nearbyVolunteers !== null ? (
                ...
                <p className="text-sm text-muted-foreground">{t('patient.noMeds') || 'Volunteer service available. In case of emergency, our network will be alerted.'}</p>
              )}
        ...
        <div className="glass-card rounded-3xl p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-semibold text-foreground flex items-center gap-2"><MessageSquare size={18} className="text-primary" /> {t('patient.messages')}</h2>
            {data.unread_messages > 0 && <span className="text-xs bg-primary/10 text-primary px-3 py-1 rounded-full">{data.unread_messages} {t('patient.newMessages')}</span>}
          </div>
          <div className="space-y-3 max-h-80 overflow-y-auto">
            {messages.length > 0 ? (
              ...
              <p className="text-center text-muted-foreground py-6">{t('patient.noMessages') || 'No messages from your doctor yet.'}</p>
            )}
          </div>
        </div>
