        ...
        <motion.div initial="hidden" animate="visible" className="space-y-6">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-0.5">{dashData.total_patients} patients under surveillance • Last sync just now</p>
            </div>
            <div className="flex items-center gap-3">
              <button onClick={() => navigate('/doctor/create-course')} className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-primary to-secondary text-primary-foreground text-sm font-medium shadow-lg shadow-primary/25 hover:scale-105 transition-all">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-7 gap-4">
            <StatCard title="Total Patients" value={dashData.total_patients} icon={Users} color="primary" trend={mockTrend} sparkline={mockSparkline} />
            <StatCard title="Critical" value={dashData.critical_count} icon={AlertTriangle} color="red" trend={-5} sparkline={[2,3,1,4,2]} />
            <StatCard title="High Risk" value={dashData.high_risk_count} icon={Activity} color="orange" trend={8} sparkline={[1,2,3,2,4]} />
            <StatCard title="Stable" value={dashData.stable_count} icon={TrendingUp} color="emerald" trend={15} sparkline={[5,6,7,8,9]} />
            <StatCard title="Recovery Rate" value="87" icon={Heart} color="purple" trend={3} suffix="%" />
            <StatCard title="AI Alerts Today" value={alerts.length} icon={Zap} color="yellow" trend={-2} />
            <StatCard title="Med Compliance" value="92" icon={Pill} color="cyan" trend={5} suffix="%" />
          </div>

          <div className="grid lg:grid-cols-5 gap-6">
            {/* Patient List */}
            <motion.div custom={5} variants={fadeUp} className="lg:col-span-2 glass-card rounded-3xl p-5 border border-border/50 flex flex-col">
              <div className="flex items-center justify-between mb-4">
                <h2 className="font-semibold text-foreground flex items-center gap-2"><Users size={16} className="text-primary" /> Assigned Patients</h2>
                <button onClick={openAddPanel} className="text-xs flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-primary/10 text-primary border border-primary/20 hover:bg-primary/20 transition-colors"><UserSearch size={13} /> Add Patient</button>
              </div>
                    ...
                    <div className="border border-primary/20 rounded-xl bg-primary/5 p-4 space-y-3">
                      ...
                      {addPanelStep === 'search' && (
                        <div className="space-y-2">
                          ...
                          <button onClick={() => navigate('/doctor/create-course')} className="w-full text-xs text-primary hover:underline flex items-center justify-center gap-1 py-1"><Plus size={11} /> Create a new course instead</button>
                        </div>
                      )}
                      ...
                      {addPanelStep === 'pick-course' && foundPatient && (
                        <div className="space-y-3">
                          ...
                          <div className="flex gap-2">
                            ...
                            <button onClick={assignCourseToPatient} disabled={!selectedCourseId || assigning} className="flex-1 py-2 rounded-lg gradient-primary text-primary-foreground text-xs font-medium disabled:opacity-40 flex items-center justify-center gap-1">
              ...
              <div className="relative mb-3">
                ...
                <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by name or ID..." className="w-full pl-9 pr-4 py-2 rounded-xl bg-muted/50 border border-border text-sm focus:ring-2 focus:ring-primary/30" />
              </div>
              <div className="flex-1 space-y-1.5 overflow-y-auto max-h-[calc(100vh-420px)]">
                ...
                {filteredPatients.length === 0 && (
                  <div className="text-center py-8 space-y-2">
                    ...
                    <button onClick={() => navigate('/doctor/create-course')} className="text-xs text-primary hover:underline flex items-center gap-1 mx-auto"><Plus size={11} /> Create your first course</button>
                  </div>
                )}
              </div>
            </motion.div>
              ...
              {detailLoading ? (
                ...
                <>
                  ...
                  <div className="grid md:grid-cols-2 gap-5">
                    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-50px" }} transition={{ duration: 0.5 }} className="glass-card rounded-3xl p-5">
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    ...
                    <div className="glass-card p-5 rounded-2xl">
                      <h3 className="text-sm font-semibold mb-3 flex items-center gap-2"><Clock size={14} className="text-primary" /> Recent Check-ins</h3>
                  ...
                  {detail.medications?.length > 0 && (
                    <div className="glass-card rounded-3xl p-5">
                      <h3 className="text-sm font-semibold mb-3 flex items-center gap-2"><Pill size={15} className="text-cyan-400" /> Medications</h3>
                      <div className="space-y-3">
                        {detail.medications.slice(0,3).map(m => (
                          <div key={m.id} className="p-3 rounded-xl bg-muted/30">
                            <div className="flex justify-between"><p className="font-medium">{m.name} {m.dosage}</p><span className={`text-xs ${m.taken ? 'text-emerald-400' : 'text-yellow-400'}`}>{m.taken ? 'Taken' : 'Pending'}</span></div>
                            <p className="text-xs text-muted-foreground mt-1">{m.frequency} · {m.time_of_day}</p>
                            <div className="w-full h-1.5 bg-muted rounded-full mt-2"><div className={`h-full ${m.taken ? 'bg-emerald-500' : 'bg-yellow-500'} rounded-full`} style={{ width: m.taken ? '100%' : '0%' }} /></div>
                          </div>
                        ))}
                      </div>
                    </div>
                          ...
                          <div key={w.id} className="p-3 bg-muted/40 rounded-lg border border-border/50 flex flex-col md:flex-row gap-4">
                            ...
                            <div className="flex-1 min-w-0 flex flex-col justify-center">
                              <div className="flex items-center justify-between gap-2 mb-1.5">
                                <h4 className="text-sm font-semibold text-foreground truncate">Severity: {w.severity}</h4>
                                <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${w.wound_score > 7 ? 'bg-red-500/10 text-red-500' : w.wound_score > 3 ? 'bg-orange-500/10 text-orange-400' : 'bg-emerald-500/10 text-emerald-400'}`}>Score: {w.wound_score.toFixed(1)}/10</span>
                              </div>
                              <p className="text-xs text-muted-foreground mb-3 leading-relaxed">{w.summary || 'No detailed summary provided.'}</p>
                              <div className="flex flex-wrap gap-1.5 mt-auto">
                                <span className={`text-[10px] px-2 py-0.5 rounded border ${w.redness ? 'border-red-400/30 bg-red-400/10 text-red-400' : 'border-border/50 text-muted-foreground'}`}>{w.redness ? 'Redness Detected' : 'No Redness'}</span>
                                <span className={`text-[10px] px-2 py-0.5 rounded border ${w.swelling ? 'border-orange-400/30 bg-orange-400/10 text-orange-400' : 'border-border/50 text-muted-foreground'}`}>{w.swelling ? 'Swelling Detected' : 'No Swelling'}</span>
                                <span className={`text-[10px] px-2 py-0.5 rounded border ${w.texture_change ? 'border-yellow-400/30 bg-yellow-400/10 text-yellow-400' : 'border-border/50 text-muted-foreground'}`}>{w.texture_change ? 'Texture Change' : 'Normal Texture'}</span>
                              </div>
                            </div>
                  ...
                  <div className="glass-card rounded-3xl p-5">
                    <div className="flex gap-3">
                      ...
                      <button onClick={handleSendMessage} disabled={sendingMsg} className="px-5 py-2.5 rounded-xl bg-primary text-primary-foreground text-sm font-medium hover:bg-primary/90 disabled:opacity-50 flex items-center gap-2"><Send size={14} /> Send</button>
                    </div>
                  </div>
                </>
              ) : (
